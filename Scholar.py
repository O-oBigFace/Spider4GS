...
import time
import warnings
from ProxyProvider import proxy
    ...
    def __init__(self, __data):
        if isinstance(__data, str):
            ...
            if citedby and citedby.text != '':
                ...
                self.citedby = 0
        self.url_picture = "/citations?view_op=view_photo&user=NMS69lQAAAAJ&citpid=%d" % random.randint(0, 10)
        self._filled = False
...
def search_photo(google_id):
    if google_id:
        if google_id[0] is not "/":
            url = _HOST + "/citations?view_op=view_photo&user=%s&citpid=%d" % (google_id, random.randint(0, 10))
        else:
            url = _HOST + google_id
